...
impl PyCogRecord {
    ...
    #[new]
    fn new(meta: PyRef<PyNumArrayU8>, cam: PyRef<PyNumArrayU8>,
           btree: PyRef<PyNumArrayU8>, embed: PyRef<PyNumArrayU8>) -> Self {
        PyCogRecord {
            inner: CogRecord::new(
                meta.inner.clone(), cam.inner.clone(),
                btree.inner.clone(), embed.inner.clone(),
            ),
        }
    }
    ...
    #[staticmethod]
    fn zeros() -> Self {
        PyCogRecord { inner: CogRecord::zeros() }
    }
    ...
    #[staticmethod]
    fn sweep_batch(query: &PyCogRecord, database: Vec<u8>, n: usize, thresholds: [u64; 4]) -> PyResult<Vec<(usize, [u64; 4])>> {
        ...
        Ok(results.into_iter().map(|r| (r.index, r.distances)).collect())
    }
    ...
    fn from_bytes(data: Vec<u8>) -> PyResult<Self> {
        if data.len() < 8192 {
            return Err(PyValueError::new_err(
                format!("CogRecord requires 8192 bytes, got {}", data.len())
            ));
        }
        Ok(PyCogRecord { inner: CogRecord::from_bytes(&data) })
    }
